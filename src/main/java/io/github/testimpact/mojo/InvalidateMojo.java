...
/**
 * Removes the shared coverage map (at the reactor root) and every reactor module's
 * per-build state (dump, selection record, build counter), forcing a cold rebuild.
 */
...
public class InvalidateMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        ReactorScope scope = new ReactorScope(session, project, new ChangeDetector(topBasedir).repoRoot());
        ...
        try {
            ...
            getLog().info("test-impact: shared coverage map and per-module state cleared ("
