...
/**
 * View over the current Maven session that answers reactor-monorepo questions:
 *   - Where is the reactor-shared coverage map?
 *   - Which reactor module owns a given (repo-root-relative) source path?
 *   - Which modules are reachable from the current one via the reactor dependency graph?
 *   - Where are all the compiled-class output dirs (for cross-module inner-class expansion)?
 *   - Does a given test class live in the current module's test classpath?
 *
 * Construct once per Mojo execution. All reads are best-effort: if Maven hands us a
 * detached project (no session, no graph), we fall back to single-module behaviour.
 */
public final class ReactorScope {
    ...
    public File repoRoot() { return repoRoot; }

    public MavenProject currentProject() { return currentProject; }

    /** Project whose basedir is the longest prefix of the given repo-root-relative path; null if none match. */
    ...
    /**
     * Pure helper: walks {@code index} (descending-length-ordered) and returns the value
     * whose key is the longest prefix of {@code path}. An empty-string key is treated as
     * a wildcard fallback. Public so the prefix logic is unit-testable without Maven.
     */
    ...
    /**
     * Restrict the change set to sources owned by modules reachable from the current one.
     * Sources outside any reactor module (e.g. top-level config files) are kept — they may
     * still be relevant and we'd rather over-select than silently miss.
     */
    ...
    public List<Path> allOutputDirs() {
        ...
        Iterable<MavenProject> projects = session == null
    ...
    /** True iff the given test class FQN has a compiled .class file under the current module's test output dir. */
    ...
    private static TreeMap<String, MavenProject> buildBasedirIndex(MavenSession session, File repoRoot) {
