...
/**
 * Bytecode transformer that:
 *   - skips system / framework / agent-internal classes;
 *   - in test classes (have any test annotations), wraps test methods with begin/end;
 *   - in non-test classes, prepends a {@link CoverageRecorder#touch(String)} call to every method.
 *
 * Two-pass on each class: first pass detects which methods are tests, second pass transforms.
 */
final class CoverageTransformer implements ClassFileTransformer {
    ...
    private static final class InjectingVisitor extends ClassVisitor {
        ...
        InjectingVisitor(ClassVisitor cv, String internalName, Set<String> testMethods, boolean isTestClass) {
            super(ASM_API, cv);
        ...
        public MethodVisitor visitMethod(int access, String name, String descriptor,
            ...
            // Skip touch injection for synthetic / bridge / <clinit> & <init> nano-methods? Keep simple: instrument all.
            return new MethodAdvice(mv, access, name, descriptor, classNameForTouch, isTest, testId, !isTestClass);
        }
    }
    ...
    private static final class MethodAdvice extends AdviceAdapter {
        ...
        MethodAdvice(MethodVisitor mv, int access, String name, String desc,
                     String classRef, boolean isTest, String testId, boolean injectTouch) {
